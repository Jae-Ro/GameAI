...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        self.cnn_layers = nn.Sequential(
            ...
            nn.Conv2d(self.layer1_inputsize, self.layer1_outputsize, kernel_size=5, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=self.maxpool_kernel, stride=2),
            # Defining another 2D convolution layer
            nn.Conv2d(self.layer2_inputsize, self.layer2_outputsize, kernel_size=5, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=self.maxpool_kernel, stride=2)
        )
    ...
    def forward(self, x):
        ...
        out = out.view(-1,self.reduced_h * self.reduced_w * self.layer2_outputsize)
        # print(out.size())
