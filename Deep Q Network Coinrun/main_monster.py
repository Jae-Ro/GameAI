...
parser.add_argument("--seed", help="which level", default=ONE_MONSTER)
...
GAMMA = 0.8               # How much to discount the future [0..1]
...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        self.layer1_inputsize = 3
        self.layer1_outputsize = 16
        self.layer2_inputsize = self.layer1_outputsize
        self.layer2_outputsize = 32

        # self.cnn_layers = nn.Sequential(
        #     # Defining a 2D convolution layer
        #     nn.Conv2d(self.layer1_inputsize, self.layer1_outputsize, kernel_size=3, stride=1, padding=1),
        #     nn.ReLU(inplace=True),
        #     nn.MaxPool2d(kernel_size=self.maxpool_kernel, stride=2),
        #     # Defining another 2D convolution layer
        #     nn.Conv2d(self.layer2_inputsize, self.layer2_outputsize, kernel_size=3, stride=1, padding=1),
        #     nn.ReLU(inplace=True),
        #     nn.MaxPool2d(kernel_size=self.maxpool_kernel, stride=2)
        # )

        # self.reduced_h = int(h/(self.maxpool_kernel*2))
        # self.reduced_w = int(w/(self.maxpool_kernel*2))

        # self.linear_layers = nn.Sequential(
        #     nn.Linear(self.reduced_h * self.reduced_w * self.layer2_outputsize, self.num_actions)
        # )


        self.conv1 = nn.Conv2d(self.layer1_inputsize, self.layer1_outputsize, kernel_size=5, stride=2)
        self.bn1 = nn.BatchNorm2d(self.layer1_outputsize)
        self.conv2 =  nn.Conv2d(self.layer2_inputsize, self.layer2_outputsize, kernel_size=5, stride=2)
        self.bn2 = nn.BatchNorm2d(self.layer2_outputsize)
        
        def conv2d_size_out(size, kernel_size = 5, stride = 2):
            return (size - (kernel_size - 1) - 1) // stride  + 1
        conv_width = conv2d_size_out((conv2d_size_out(w)))
        conv_height = conv2d_size_out((conv2d_size_out(h)))
        linear_input_size = conv_width * conv_height * self.layer2_outputsize
...
def initializeOptimizer(parameters):
    ...
    optimizer = optim.SGD(parameters, lr=0.3, momentum = 0.5)
    ### WRITE YOUR CODE ABOVE HERE
    return optimizer
...
def select_action(state, policy_net, num_actions, epsilon, steps_done = 0, bootstrap_threshold = 0):
    ...
    if steps_done > bootstrap_threshold and new_epsilon >=0:
      if new_epsilon >=0.56:
        new_epsilon -= 0.0001
      else:
        new_epsilon = 0.56
    ...
    if rand > new_epsilon:
      ...
      action = torch.tensor([[rand_action]],  device=DEVICE, dtype=torch.long)
...
def doMakeBatch(replay_memory, batch_size):
    ...
    for s in batch.next_state:
      if s is not None:
        arr_mask.append(True)
      else:
        arr_mask.append(False)
    
    # [state is not None for s in batch.next_state]
    # [state for state in batch.next_state if state is not None]
    non_final_mask = torch.tensor(list(arr_mask), device=DEVICE, dtype=torch.uint8)
  
    arr_next = []
    for state in batch.next_state:
      if state is not None:
        arr_next.append(state)

    next_states_batch = torch.cat(arr_next)
