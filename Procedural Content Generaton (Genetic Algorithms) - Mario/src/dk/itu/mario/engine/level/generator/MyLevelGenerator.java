...
public class MyLevelGenerator {
	...
	private MyDNA generateRandomIndividual() {
		...
		for (int i = 0; i < chromLength; i++) {
			...
			chromosome += chars.charAt(randLetterIdx);
			if (chromosome.charAt(chromosome.length() - 1) == 'G') {
				randNumber = rand.nextInt(3);
			}
			chromosome += randNumber;
		}
		// System.out.println("LEVEL: " + chromosome);
		individual.setChromosome(chromosome);
		individual.setNumGenes(chromosome.length() / 2);
	...
	private boolean terminate(ArrayList<MyDNA> population, int count) {
		...
		for (MyDNA dna : population) {
			...
			if (fitness >= 0.81 || count > maxIterations) {
				decision = true;
			}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation(ArrayList<MyDNA> population) {
		...
		int randSize = rand.nextInt(popSize);
	...
	// Modifying this function is optional, and only need be done if you have
	// modified numberOfCrossovers()
	...
	// Modifying this function is optional and only needed if you've changed
	// competeWithParentsOnly() to return true
	private ArrayList<MyDNA> competeWithParents(ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation,
			Hashtable parents) {
	...
	// Combine the old population and the new population and return the top fittest
	// individuals.
	// You will complete this function unless you've changed
	// competeWithParentsOnly() and competeWithParents
	private ArrayList<MyDNA> globalCompetition(ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation) {
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		ArrayList<MyDNA> testPop = new ArrayList<MyDNA>();
		testPop.addAll(oldPopulation);
		testPop.addAll(newPopulation);
		int popSize = oldPopulation.size();

		while (finalPopulation.size() < popSize) {
			MyDNA best = getBestIndividual(testPop);
